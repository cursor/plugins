...
- **Rules**
  - `cloudflare-workers.mdc` — best practices for Cloudflare Workers including Web Standard APIs, error handling, storage selection (KV, D1, R2, Durable Objects, Queues), CPU time limits, `waitUntil()`, CORS, environment bindings, module Worker syntax, HTMLRewriter streaming transformations, and Cache API edge caching
  - `cloudflare-d1.mdc` — comprehensive D1 database best practices covering parameterized queries, batch operations, prepared statements, error handling, migrations, edge query optimization, consistency model, SQLite data types, and pagination patterns
...
- **Agent**
  - `cloudflare-workers-agent.md` — Worker development agent for creating, debugging, and optimizing Cloudflare Workers, with guidance on architecture patterns and storage selection
- **Skills**
