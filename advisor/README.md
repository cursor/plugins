...
This is the Cursor counterpart of Claude Code's `/advisor`, and because the advisor is just a subagent with its own model, the two models do not have to be from the same family: run Claude with a GPT advisor, or GPT with an Opus advisor.
...
```text
/advisor                     turn on with the default advisor (Opus)
/advisor gpt                 turn on with a GPT advisor
...
```text
Advisor (claude-opus-5-thinking-xhigh): proceed with changes. The retry wrapper hides the
real failure; the 401 comes from a stale token cache. Dropped the retry, fixed the cache key.
```
...
```mermaid
flowchart LR
  U[User] -->|/advisor opus| M[Main model]
...
Any other value is passed through as a full slug. If Cursor rejects a slug, the skill picks the closest valid one from the error, saves it, and tells you. Team model restrictions and plan limits apply to the advisor like any subagent.
...
Each consult is one call to the advisor model with a briefing of a few thousand tokens plus whatever the advisor chooses to read. Selective use is the point: a typical feature takes one to three consults. Pick a cheaper alias if cost matters more than depth.
