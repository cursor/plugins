...
6. For a big or wide change, run it as `$pstack:arena`. Use Codex's `spawn_agent` semantics with `xhigh` reasoning to ask several agents the same question, then merge the answers. Route architecture, complex bug, performance, hillclimb, synthesis, and judge work to Sol.
