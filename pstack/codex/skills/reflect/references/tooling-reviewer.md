...
Review the active-thread material supplied below. It may be content returned by Codex `read_thread`, an active-context digest, or a transcript path explicitly supplied by the user. Never guess a transcript path.
...
- `spawn_agent` or `followup_task` messages that name a skill path or skill name
