...
#
# Transcript lookup is opt-in. Pass the second argument or set
# POTETO_TRANSCRIPTS_DIR when an explicitly supplied transcript directory is
# in scope. The audit never discovers a host application's private transcripts.
set -u
...
transcripts="${POTETO_TRANSCRIPTS_DIR:-${2:-}}"
now=$(date +%s)
...
git worktree list --porcelain | awk '/^worktree /{print $2}' | while read -r wt; do
	...
	# Most recent explicitly supplied transcript that operated in this worktree.
	# Match path followed by "/" or a quote so glint-482 does not match
	# glint-482-r37.
	last="-"; last_ts=0
