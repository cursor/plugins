...
When verifying work from Codex's `spawn_agent` with `xhigh` reasoning, inspect the actual output artifact (git diff, file contents, runtime behavior), not the agent's summary. Agents report what they intended, not always what happened.
