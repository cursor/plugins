...
Write `~/.cursor/rules/pstack-models.mdc` with `alwaysApply: true`, a `# budget` line, and one line per role, using the same labels poteto-mode uses. Overwrite the whole file so re-runs stay idempotent. Shape:
