...
4. Pick the worker model from `swarm workers` in the pstack config file (`[environment].rules_file`; default `~/.cursor/rules/pstack-models.mdc`) when present. Otherwise use `grok-4.6-fast-xhigh`. For a model race, name each arm's model up front.
