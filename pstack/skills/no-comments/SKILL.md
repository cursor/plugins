...
1. Delegate the scope to the installed **Comment Sicko** agent through the host's native delegation feature. Do not restate its rules. If named agents are unavailable, resolve the optional Comment Sicko adapter file at `../../agents/comment-sicko.md` relative to this skill directory and read it before using it as a general delegate's instructions. A skills-only installation may omit that adapter. If neither the named agent nor its instruction file is available, report `COMMENT-REVIEW UNAVAILABLE` and stop without deleting comments. If the instructions are available but delegation is not, apply them yourself and report that review independence was unavailable.
