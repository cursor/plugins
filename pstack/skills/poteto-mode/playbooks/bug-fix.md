...
4. Verify on the same surface. Rerun the exact baseline input against the patched build and observe the expected final state through the full chain. For an integration fix, cross the changed provider or service seam. For a write-capable path, use a named sandbox, disposable target, or dry-run. "Inconclusive," lower-level-only, or wrong-surface evidence is not a pass; flag it as `behavior-unverified`. Unit tests show branch behavior, not bug absence.
...
**Reply:** what was broken, root cause, fix, and how you verified it on the same surface. Include the baseline and patched inputs, environments, and observed outputs. Paste failing-then-passing repro output verbatim. Do not report the fix as done with only automated-test evidence.
