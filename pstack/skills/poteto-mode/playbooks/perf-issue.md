...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation and apply the confirmed `perf-issue` mapping from `~/.config/pstack/models.md` when supported; otherwise inherit the parent model. If delegation is unavailable, implement directly and record the missing review separation. Review the diff. Capture a post-fix trace.
