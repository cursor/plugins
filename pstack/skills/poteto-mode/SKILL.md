...
- An explicit local-lab request, or a change that crosses deployed services, runtime protocols, durable ownership, upgrade or migration paths, hardware adapters, physical actuation, or production-readiness gates → the **Local Lab Validation** playbook (`playbooks/local-lab-validation.md`) after the final mutation and before calling the work production-ready or starting Shipping. An open PR may carry a pending or blocked lab verdict. Do not trigger it only because one local change makes many packages compile.
...
- **Local Lab Validation.** Production-readiness proof for changes that cross deployed services, runtime seams, upgrades, or physical hardware. `playbooks/local-lab-validation.md`.
