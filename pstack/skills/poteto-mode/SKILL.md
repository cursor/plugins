...
- Jira, Confluence, or any Atlassian Cloud ticket URL → Cursor's built-in browser (`cursor-ide-browser`). Call `browser_navigate` on the ticket URL, then `browser_lock`, then `browser_snapshot`. Read the title, description, comments, and attachments from the snapshot. Download ticket attachments when they are the evidence. WebFetch and curl return the SPA JS shell on these URLs, not the ticket. Do not use them here. This is ticket reading, not product-UI verification. Keep `control-ui` for the app you are shipping.
