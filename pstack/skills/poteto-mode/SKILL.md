...
For an explicit goal with on-demand delegation, use the [goal skill](../goal/SKILL.md). Its role table and delegation policy take precedence over the defaults below for that run.
...
- **Goal with subagents.** An explicit goal split into bounded tasks with on-demand delegation. Use the [goal skill](../goal/SKILL.md) for model selection, dispatch, and root verification.
