...
Read the transcripts root from `[environment].transcripts_dir` in the pstack config file (written by `/setup-pstack`). With no such file or section, the following Cursor default applies, including its slug scheme. Transcripts live at `~/.cursor/projects/<slug>/agent-transcripts/<uuid>/<uuid>.jsonl`, where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-" (so `/Users/you/proj` becomes `Users-you-proj`). Every line is one chat message. Other hosts use their own transcript layout as named by the system prompt or setup-pstack output.
