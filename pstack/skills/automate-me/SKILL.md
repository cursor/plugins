...
Look recursively for `<skills_output_dir>/**/*-mode/SKILL.md` — the skills directory from `[environment].skills_output_dir` in the pstack config file, default `.cursor/skills/` — plus the user-level equivalent (`~/.cursor/skills/*-mode/SKILL.md` on Cursor) matching the user's handle. Mode skills can live in a personal category directory (`<skills_output_dir>/<handle>/`, default `.cursor/skills/<handle>/`), not only at the top level. If one exists, confirm intent with `AskQuestion` (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out. The system prompt names the workspace's `agent-transcripts/` directory. Use only that path. Don't glob across sibling workspaces under the transcripts root (`[environment].transcripts_dir` in the pstack config file; default `~/.cursor/projects`). That crosses workspace boundaries and reads private chats from unrelated projects.
...
- Path: preserve an existing mode skill's category. For a new mode, use `<skills_output_dir>/<handle>/<handle>-mode/SKILL.md` when the repo has an established personal category for that handle; otherwise default to `<skills_output_dir>/<handle>-mode/SKILL.md` in the project — both from `[environment].skills_output_dir` in the pstack config file, default `.cursor/skills/` (or the user-level skills directory's `<handle>-mode/` — on Cursor `~/.cursor/skills/<handle>-mode/` — if the user prefers a personal skill).
